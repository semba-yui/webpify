...
- [品質とファイルサイズの最適化](#品質とファイルサイズの最適化)
  ...
  - [lossless（可逆圧縮）変換](#lossless可逆圧縮変換)
- [出力先の管理](#出力先の管理)
...
元の画像データを完全に保持したい場合に使用します。

```bash
# lossless モードで変換
webpify ./originals --lossless

# ディレクトリを再帰的に lossless 変換
webpify ./archive -r --lossless
```

**用途**:

- **画像アーカイブ**: 原本を品質劣化なく保存
- **デザイン資産の保存**: 編集用のマスターファイル保持
- **PNG からの変換**: 透過情報を含む画像の完全保持
- **医療・科学画像**: 精度が重要な画像の変換

**注意点**:

- ファイルサイズは元の画像より大きくなる場合があります（特に JPEG からの変換時）
- `--quality` オプションと同時に指定した場合、警告が表示され lossless が優先されます

#### lossless 変換の比較例

```bash
# 通常変換（lossy）
webpify photo.jpg -q 100
# photo.jpg (4.92 MB) → photo.webp (2.45 MB)

# lossless 変換
webpify photo.jpg --lossless
# photo.jpg (4.92 MB) → photo.webp (6.85 MB)
```
